...
const postToInstagram = async (imageUrls, caption) => {
    ...
    if (imageUrls.length === 1) {
        ...
        const carouselRes = await axios.post(carouselContainerUrl, {
            caption: caption, media_type: 'CAROUSEL', children: childContainerIds, access_token: accessToken,
        });
    ...
    try {
        ...
        if (igPostId) {
            ...
            if (entryIndex !== -1) {
                ...
                db[entryIndex].status = 'active';
                db[entryIndex].productTitle = product.title;
            } else {
                db.push({ 
...
// ==========================================================
// ============== 2. تصدير جميع الدوال في النهاية ==============
// ==========================================================
module.exports = { 
    createPost, 
